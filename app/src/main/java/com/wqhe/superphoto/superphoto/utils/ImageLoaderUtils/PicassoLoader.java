...
public class PicassoLoader implements ILoaderStrategy {
    ...
    private static Picasso getPicasso() {
        if (sPicassoSingleton == null) {
            synchronized (PicassoLoader.class) {
                if (sPicassoSingleton == null) {
                    sPicassoSingleton = new Picasso.Builder(SuperPhotoApplication.gApp).memoryCache(sLruCache).build();
                }
            }
        }
    ...
    public void clearDiskCache() {
        File diskFile = new File(SuperPhotoApplication.gApp.getCacheDir(), PICASSO_CACHE);
        if (diskFile.exists()) {
            //这边自行写删除代码
